...
class MapsDB:
    ...
    def get_Mapp_from_db(self, name: str):
        ...
        if dic is not None:
            m = Mapp(dic['filename'],dic['address'], dic['latitude'], dic['longitude'], dic['description'])
            m.markers = dic.get('markers', None)
            return m
        else:
            return None
...
class GeoTest:
    ...
    @staticmethod
    def make_folium_map(loc1: geopy.location.Location, mfile:str, desc=None, markers=None, tiles='OpenStreetMap',
    ...
    @staticmethod
    def make_folium_usffc_map(addr='USA', mfile=None):
        loc, lstr = GeoTest.get_single_location(addr)
        df = USffc.from_json()
        df['point2'] = df.point.apply(lambda x: (x[0], x[1]))
        markers = []
        for i in list(range(len(df))):
            row = df.iloc[i]
            m = Marker(row.latitude, row.longitude, addr=row.geo_address, name=row.code)
            # print('tooltip: {}'.format(m.tooltip))
            markers.append(m)
        map = folium.Map(location=[loc.latitude, loc.longitude], zoom_start=12, tiles="OpenStreetMap")
        tooltip = "Click Me!"
        for mark in markers:
            folium.Marker(
                [mark.latitude, mark.longitude], popup=mark.popup, tooltip=tooltip
            ).add_to(map)
        map.save(os.path.join(Config.templates_dir, mfile))
        mapp = Mapp(mfile,addr=loc.address, latitude=loc.latitude, longitude=loc.longitude,desc='Map of US Amazon Fulfilment Centers')
        mapp.set_markers(markers)
        db = MapsDB()
        db.add_mapp(mapp)
        db.save_mapsDB()
        return mfile